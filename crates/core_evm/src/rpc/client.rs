...
use core_jsonrpc::client::JsonRpcClient as GenericJsonRpcClient;
use core_jsonrpc::types::{ERROR_INTERNAL_ERROR, JsonRpcError, JsonRpcResult};
...
use super::{
    ankr::AnkrClient,
    model::{
        Block, BlockTransactionsIds, EthSyncingStatus, Transaction, TransactionReciept,
...
impl<C: Client + Clone> EthereumClient<C> {
    ...
    pub async fn call<T: DeserializeOwned + 'static>(
        &self,
        method: String,
    ...
    pub async fn batch_call<T: DeserializeOwned + 'static>(
        &self,
        calls: Vec<(String, serde_json::Value)>,
    ) -> Result<Vec<JsonRpcResult<T>>, JsonRpcError> {
        Ok(self
            .client
            .batch_call::<T>(calls)
            .await?
    ...
    pub async fn get_block_receipts(
        &self,
        block_number: u64,
    ) -> Result<Vec<TransactionReciept>, JsonRpcError> {
    ...
    pub async fn get_blocks(
    ...
    ) -> Result<Vec<BlockTransactionsIds>, JsonRpcError> {
        let calls: Vec<(String, serde_json::Value)> = blocks
            .iter()
            .map(|block| {
                (
                    "eth_getBlockByNumber".to_string(),
                    json!([block, include_transactions]),
                )
            })
            .collect();
        ...
        let results = self
            .client
            .batch_call::<BlockTransactionsIds>(calls)
            .await?;
    ...
    > {
        ...
        let block_ids = reciepts
            .iter()
            .map(|x| x.block_number.to_string())
            .collect::<Vec<String>>();
    ...
    pub async fn get_transactions_by_hash(
        ...
        let calls: Vec<(String, serde_json::Value)> = hashes
            .iter()
            .map(|hash| ("eth_getTransactionByHash".to_string(), json!([hash])))
            .collect();
    ...
    pub async fn get_transactions_receipts(
        ...
        let calls: Vec<(String, serde_json::Value)> = hashes
            .iter()
            .map(|hash| ("eth_getTransactionReceipt".to_string(), json!([hash])))
            .collect();
        Ok(self
            .client
            .batch_call::<TransactionReciept>(calls)
            .await?
            .extract())
    }
    ...
    pub async fn get_transaction_receipt(
        &self,
        hash: &str,
    ) -> Result<TransactionReciept, JsonRpcError> {
    ...
    pub async fn trace_replay_block_transactions(
        &self,
        block_number: u64,
    ) -> Result<Vec<TransactionReplayTrace>, JsonRpcError> {
    ...
    pub async fn trace_replay_transactions(
        &self,
        tx_hash: &[String],
    ) -> Result<Vec<TransactionReplayTrace>, JsonRpcError> {
        let calls: Vec<(String, serde_json::Value)> = tx_hash
            ...
            .map(|hash| {
                (
                    ...
                    json!([hash, json!(["stateDiff"])]),
                )
            })
            .collect();
        Ok(self
            .client
            .batch_call::<TransactionReplayTrace>(calls)
            .await?
            .extract())
    }
    ...
    pub async fn batch_eth_call<const N: usize>(
        ...
        let calls: Vec<(String, serde_json::Value)> = function_selectors
            ...
            .map(|selector| {
                (
                    "eth_call".to_string(),
                    json!([{"to": contract_address, "data": selector}, "latest"]),
                )
            })
            .collect();
        ...
        results
            .try_into()
            .map_err(|_| "Array conversion failed".into())
    }
    ...
    pub async fn multicall3(
        &self,
        calls: Vec<Call3>,
    ) -> Result<Vec<MulticallResult>, Box<dyn std::error::Error + Sync + Send>> {
